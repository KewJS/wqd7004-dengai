...
**Check the data type of the data set and normalize it. In this data set, the date datatype had been formalized to Date data type**
...
**Combine Train features with train label to understand the data more precisely **
...
We can see that 'sj' city (San Juan) has more data than 'iq'(Iquitos).
...
```{r}
train <-  read.csv("https://raw.githubusercontent.com/KewJS/wqd7004-dengai/main/data/Train_cleaned.csv") 
```
...
iq_train_reg <- subset(iq, for_splitting == TRUE)
iq_test_reg <- subset(iq, for_splitting == FALSE)
```

```{r iq sanity check}
nrow(iq_train_reg)+nrow(iq_test_reg) == nrow(iq)
```

```{r sj split data}
set.seed(101)
for_splitting <- sample.split (Y = sj$total_cases, SplitRatio = 0.7)
sj_train_reg <- subset(sj, for_splitting == TRUE)
sj_test_reg <- subset(sj, for_splitting == FALSE)
```

```{r sj sanity check}
nrow(sj_train_reg)+nrow(sj_test_reg) == nrow(sj)
```
# IQ regression model
...
```{r}
iq_df<- df_predict[which(df$city=="iq"),]
sj_df<- df_predict[which(df$city=="sj"),]
