...
a) Build a classification model to predict the occurence of dengue outbreak in San Juan and Iquitos city.

b) Build a regression model to predict weekly dengue cases in San Juan and Iquitos city. 
...
The diagram below show the performance of classification model and regression model using different algorithm: 

### Classification Model

```{r}
knitr::include_graphics('E:/UM/R programming/1.PNG')
```

### Regression Model
```{r}
knitr::include_graphics('E:/UM/R programming/2.PNG')
```

Based on the result, we observed that Classification Model is performing better than Regression Model. The algorithm being used in regression models are not powerful enough to predict the total cases that might happened because the algorithms is not capable enough to handle the complexity of the dataset. A further study is needed to configure the extra step needed to carry out and a more complex algorithm to handle the data to increase the overall performance.